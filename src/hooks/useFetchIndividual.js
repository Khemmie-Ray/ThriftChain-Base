...
const useFetchIndividual = () => {
  const { allSingle, singleUser } = useThriftData(); // still coming from context, possibly Proxy
  ...
  const decodeThrift = (res) => {
    ...
    const arr = Array.from(raw); // convert Proxy to array
  ...
  const fetchSingle = useCallback(async () => {
    ...
    try {
      ...
      const decodedAll = resultAll.map(decodeThrift);
      const decodedUser = resultUser.map(decodeThrift);
